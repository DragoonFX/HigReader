...
for file in "${entries[@]}";
do
	if [[ "$file" != "$script_dir"/*_op.txt && "$file" != "$script_dir"/*vm*.txt && "$file" != "$script_dir"/flow.txt && "$file" != "$script_dir"/init.txt && "$file" != "$script_dir"/dummy.txt && "$file" != "$script_dir"/\&*.txt && "$file" != "$script_dir"/omake*.txt && "$file" != "$script_dir"/*choicesdialog.txt && "$file" != "$script_dir"/staffroom.txt ]];
	then
		echo "$file"
